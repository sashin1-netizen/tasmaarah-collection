...
if(gallery.length){
  ...
  box.innerHTML='<button class="lightbox-close" type="button" aria-label="Close">×</button><button class="lightbox-prev" type="button" aria-label="Previous">‹</button><figure><img alt=""><figcaption></figcaption></figure><button class="lightbox-next" type="button" aria-label="Next">›</button>';
...
/* Pointer-reactive lighting only changes CSS variables; it never creates overlay elements. */
const reduceMotion=window.matchMedia('(prefers-reduced-motion: reduce)').matches;
if(!reduceMotion){
  const updateAmbient=(x,y)=>{
    document.documentElement.style.setProperty('--tx-x',`${Math.max(0,Math.min(100,(x/window.innerWidth)*100)).toFixed(1)}%`);
    document.documentElement.style.setProperty('--tx-y',`${Math.max(0,Math.min(100,(y/window.innerHeight)*100)).toFixed(1)}%`);
  };
