(()=>{
  ...
  /* Art direction is an enhancement; the base site remains usable without JS. */
  if(!document.querySelector('[data-tasmaarah-art]')){
    const art=document.createElement('link');
    art.rel='stylesheet';art.href='atelier-client-ready.css?v=10';art.dataset.tasmaarahArt='true';
  ...
  /* Page progress gives the long editorial pages a deliberate sense of movement. */
  ...
  /* Images begin loading well before they enter the viewport, but only the hero receives high priority. */
  ...
  if('IntersectionObserver'in window){
    const imageObserver=new IntersectionObserver(entries=>entries.forEach(entry=>{if(!entry.isIntersecting)return;const img=entry.target;img.loading='eager';img.fetchPriority='low';imageObserver.unobserve(img)}),{rootMargin:'120% 0px 120% 0px',threshold:0});
    lazy.forEach(img=>imageObserver.observe(img));
  }

  /* Content is always visible; reveals never gate access. */
  ...
  /* Collection search + useful filter chips generated from genuine categories already on the page. */
  ...
  /* Quote form becomes a live WhatsApp brief. */
  ...
  /* Gallery lightbox with keyboard + swipe. */
  ...
  /* Restrained desktop lighting only. */
